defmodule MiniComponents.Attributes do
  ...
  def do_set_attributes(assigns, attributes, opts \\ []) do
    new_assigns =
      ...
      |> Enum.reduce(%{}, fn attr, acc ->
        ...
        attr_key = prop_attribute_key(attr)
  ...
  defp prop_attribute(attr), do: attr |> to_string() |> String.replace("_", "-")
  defp data_attribute(attr), do: "data-#{prop_attribute(attr)}"

  defp attribute_key_and_default({attr, default}), do: {attr, default}
  defp attribute_key_and_default(attr), do: {attr, nil}

  defp prop_attribute_key(attr) do
  ...
  defp handle_into_option(assigns, into) do
    ...
    attr_key = prop_attribute_key(into)
